...
uint32_t access_count=0, iter_count=1;
extern uint32_t global_set;
uint32_t warmup_end;
uint64_t l2pf_access = 0;
uint64_t dcounter=400000;
uint64_t prevllc=0;
...
void CACHE::build_map() {
    access_count++;
    prevllc++;
    int switch_count=0;
    if(NAME=="LLC") {
        uint64_t MEAN_HIT=0, MEAN_MISS=0, VAR_MISS=0, VAR_HIT=0;
        for(uint32_t j=0; j<NUM_SET; j++) {
            MEAN_HIT+=set_access[j][0];
            MEAN_MISS+=set_access[j][1];
            VAR_HIT+=(set_access[j][0])*(set_access[j][0]);
            VAR_MISS+=(set_access[j][1])*(set_access[j][1]);
        }
        ...
        VAR_HIT/=NUM_SET;
        VAR_MISS/=NUM_SET;
        VAR_HIT-=MEAN_HIT*MEAN_HIT;
        VAR_MISS-=MEAN_MISS*MEAN_MISS;
        int d1, d2, H, M;
        queue<int> VHq, Hq, Cq, VCq;
        for(uint32_t j=0; j<NUM_SET; j++) {
            d1=set_access[j][0]-MEAN_HIT;
            d2=set_access[j][1]-MEAN_MISS;
            // if(iter_count==0) set_access[j][0]=set_access[j][1]=0;
            H=M=1;
            // H=d1>0?2:0;
            // M=d2>0?2:0;
            if(d1*d1>=VAR_HIT/4) H=d1>0?2:0;
            if(d2*d2>=VAR_MISS/4) M=d2>0?2:0;
            // if(d2*d2>VAR_MISS/2) M=d2>0?4:0;
            // cout<<"SET: "<<j<< "\t" << "TYPE: "<<H+M<<endl;
            // cout<<H+M<<endl;
            set_access[j][3]=H+M;
            re_map[j]=j;
            switch(H+M) {
        ...
        // for(auto i:re_map) cout<<i.first<<" "<<i.second<<endl;
        // iter_count=1;
        cout<<"MH: "<<MEAN_HIT<<" MM: "<<MEAN_MISS<<" VH: "<<VAR_HIT<<" VM: "<<VAR_MISS<<" "<< access_count <<" "<<switch_count<<endl;
    }
}
...
{      if(warmup_end==1){ 
      if(NAME=="LLC") {
        // cout<<access_count<<" "<<access_count%10000<<endl;
        if(!(access_count%(dcounter/5)) && dcounter>50000) {
          build_map();
        }
      }
}      
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    if(NAME=="LLC") {
      ...
      for (uint32_t way=0; way<to_way; way++) {
      ...
      if(alter_set!=set && set_type>2 && match_way==-1) {
        if(set_access[alter_set][3]==0) extra_way=12;
        if(set_access[alter_set][3]==1) extra_way=14;
        for (uint32_t way=extra_way; way<NUM_WAY; way++) {
          if (block[alter_set][way].valid && (block[alter_set][way].tag == packet->address)) {
              global_set=alter_set;
              match_way = way;
